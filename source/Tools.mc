...
module Tools {
	...
	function getValueByFieldType(type, data){
		...
		if (type == FIELD_TYPE_HUMIDITY){
			...
			res = [Tools.momentToString(new Time.Moment(data["sunrise"])), Application.loadResource(Rez.Strings.Sunrise)];
		}else if (type == FIELD_TYPE_SUNSET){
			res = [Tools.momentToString(new Time.Moment(data["sunset"])), Application.loadResource(Rez.Strings.Sunset)];
		}else if (type == FIELD_TYPE_SUNEVENT){
			res = sunEvent(data);
			...
			res = [data["uvi"].format("%.2f"), Application.loadResource(Rez.Strings.Uvi)];
		}else if (type == FIELD_TYPE_VISIBILITY){
			res = [Tools.distanceToString(data["visibility"].format("%d")), Application.loadResource(Rez.Strings.Visibility)];
		}else if (type == FIELD_TYPE_DEW_POINT){
			res = [data["dew_point"].format("%d")+"°", Application.loadResource(Rez.Strings.DewPoint)];
		}
		return res;
	}

	///////////////////////////////////////////////////////////////////////////
	function sunEvent(data){
		...
		if (now.lessThan(sunrise)){
			return [Tools.momentToString(sunrise), Application.loadResource(Rez.Strings.Sunrise)];
		}else if (now.lessThan(sunset)){
			return [Tools.momentToString(sunset), Application.loadResource(Rez.Strings.Sunset)];
		}
		return [Tools.momentToString(sunrise), Application.loadResource(Rez.Strings.Sunrise)];
	}
	...
	function getWindColor(windSpeed){
		return ToolsGlance.getWindColor(windSpeed);
	}
	...
	function windSpeedConvert(rawData){
		return ToolsGlance.windSpeedConvert(rawData);
	}
